<template>
  <div>
    ...
    <div class="order-item" v-for="item of orderList" :key="item.id" @click="toPage(item)" v-show="item.orderStatus !== 5">
        <div class="item-top">
        ...
        <!-- <div class="item-more" v-show="getUserType === 3">
            ...
            <div class="btn" @click.stop="evaluate(item)"
              v-show="item.orderStatus === 2 || item.orderStatus === 3">
                <span>评价</span>
            </div>
        ...
        <div class="item-more" v-show="getUserType === 2">
            <div class="btn" @click.stop="storeChangeOrderType(item, 5)"
              v-show="item.orderStatus === 0 || item.orderStatus === 1 || item.orderStatus === 2">
                <span>取&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;消</span>
            </div>
...
export default {
  ...
  methods: {
    // 去到订单详情
	  toPage (data) {
			this.$router.push({path: '/manager-order-detail', query: data})
    },
    // 去到订单评价
		evaluate (data) {
			console.log(data)
